...
var dnsUrls={a9:'https://c.amazon-adsystem.com/aax2/apstag.js',prebid:'https://static.amp.services/prebid2.35.0.js',gpt:'https://securepubads.g.doubleclick.net/tag/js/gpt.js',config:'https://cdn.jsdelivr.net/gh/streamAMP/client-configs@latest/'+publisher+'.min.js'};window.streamampConfig=window.streamampConfig||{};window.streamampConfig.adUnits=window.streamampConfig.adUnits||{};window.streamampConfig.cmp=window.streamampConfig.cmp||[];window.gptAdSlots={};window.pbjs=window.pbjs||{};window.pbjs.que=window.pbjs.que||[];var googletag=googletag||{};googletag.cmd=googletag.cmd||[];window.AD_UNITS_TOGGLE_OFF=window.AD_UNITS_TOGGLE_OFF||[];window.AD_UNITS_TOGGLE_ON=window.AD_UNITS_TOGGLE_ON||[];streamampAddDNSPrefetch(Object.values(dnsUrls));if(publisher){var loadStreamampConfig=document.createElement('script');loadStreamampConfig.type='text/javascript';loadStreamampConfig.async=true;loadStreamampConfig.src=dnsUrls.config;loadStreamampConfig.onload=streamampSetup
...
pbjs.setConfig({filterSettings:{iframe:{bidders:'*',filter:'include'}}});streamampUtils.logPbjs('Queuing setConfig() for price granularity')
pbjs.setConfig({priceGranularity:generatePriceGranularity(streamampConfig.pbjsPriceGranularity)});streamampUtils.logPbjs('Queuing setConfig() for bidder timeout',streamampConfig.bidTimeout)
pbjs.setConfig({bidderTimeout:streamampConfig.bidTimeout});streamampUtils.logPbjs('Queuing setConfig() for size config (breakpoints)',streamampConfig.breakpoints.map(function(breakpoint){return breakpoint.label}))
...
apstag.fetchBids({slots:adUnitsAPS,timeout:bidTimeout},function(bids){streamampUtils.logAps('Bids received',bids)
headerBidderBack('a9');});}
...
function streamampShouldShowAddUnit(adUnitCode){if(window.AD_UNITS_TOGGLE_ON.length){return window.AD_UNITS_TOGGLE_ON.indexOf(adUnitCode)!==-1;}else{return window.AD_UNITS_TOGGLE_OFF.indexOf(adUnitCode)===-1;}}
...
if(streamampConfig.a9Enabled){apstag.fetchBids({slots:apstagSlots,timeout:bidTimeout},function(bids){streamampUtils.logAps('Bids received',bids)});}
